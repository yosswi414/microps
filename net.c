...
int net_timer_handler(void){
    ...
    for (timer = timers; timer; timer=timer->next){
        ...
        if(timercmp(&timer->interval, &diff, <) != 0){
            ...
            timer->handler();
            timeradd(&timer->interval, &timer->last, &timer->last);
            // Exercise 16-2
        }
    }
    return 0;
...
int net_event_subscribe(void (*handler)(void* arg), void* arg) {
    ...
    if(!(event = memory_alloc(sizeof(*event)))){
        errorf("memory_alloc() failed");
        return -1;
    }
...
void net_raise_event(void) {
    intr_raise_irq(INTR_IRQ_EVENT); // イベント用割り込み発生
}
